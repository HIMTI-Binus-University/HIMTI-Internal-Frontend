...
describe("event package hooks", () => {
  ...
  it("lists packages with an encoded sub-event ID", async () => {
    ...
    const { result } = renderHook(() => useEventPackages("sub/event"), {
      wrapper,
    });
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
    expect(mockedClient.get).toHaveBeenCalledWith(
      "/api/internal/sub-events/sub%2Fevent/packages",
    );
  });
  ...
  it("creates and revision-updates packages through contract paths", async () => {
    ...
    const create = renderHook(() => useCreateEventPackage("sub-1"), {
      wrapper,
    });
    const update = renderHook(() => useUpdateEventPackage("sub-1"), {
      wrapper,
    });
    const payload = {
      code: "TEAM",
    ...
    await act(() =>
      update.result.current.mutateAsync({
        packageId: "pkg/1",
        payload: { revision: 2, status: "INACTIVE" },
      }),
    );
    expect(mockedClient.post).toHaveBeenCalledWith(
      "/api/internal/sub-events/sub-1/packages",
      payload,
    );
    expect(mockedClient.put).toHaveBeenCalledWith(
      "/api/internal/event-packages/pkg%2F1",
