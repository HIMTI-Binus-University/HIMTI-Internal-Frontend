...
type EventStatus = LifecycleStatus;
type SubeventStatus = LifecycleStatus;
...
const assignedForms = (data: EventData, subeventId: string) =>
  data.subeventForms.flatMap((assignment) => {
    ...
    const version = data.formVersions.find(
      (item) => item.id === assignment.formVersionId,
    );
    const form = data.forms.find((item) => item.id === version?.formId);
    return form ? [{ assignment, form }] : [];
  });

export const ticketIsSoldOut = (
  ticket: Pick<TicketOption, "id" | "capacity">,
  registrations: Pick<Registration, "ticketOptionId" | "status">[],
) =>
  ticket.capacity !== undefined &&
  registrations.filter(
    (item) =>
      item.ticketOptionId === ticket.id &&
      !["DRAFT", "CANCELLED"].includes(item.status),
  ).length >= ticket.capacity;

export const subeventOpenBlockers = (
  data: EventData,
  subevent: Pick<PrototypeSubevent, "id">,
): string[] => {
  const tickets = data.ticketOptions.filter(
    (item) => item.subeventId === subevent.id,
  );
  const hasOpenTicket = tickets.some(
    (ticket) =>
      ticket.status === "OPEN" && !ticketIsSoldOut(ticket, data.registrations),
  );
  const requiredFormsOpen = assignedForms(data, subevent.id)
    .filter(({ assignment }) => assignment.isRequired)
    .every(({ form }) => form.status === "OPEN");
  return [
...
export const eventOpenBlockers = (
  data: EventData,
  event: Pick<PrototypeEvent, "id">,
) =>
  data.subevents.some(
    (subevent) =>
      subevent.eventId === event.id &&
      subevent.status === "OPEN" &&
      subeventOpenBlockers(data, subevent).length === 0,
  )
    ? []
...
export const statusActions = (..._args: unknown[]) =>
  statuses as unknown as EventStatus[];
...
const stamp = <T extends { updatedAt?: string; updatedBy?: string }>(
  item: T,
  now: string,
): T => ({ ...item, updatedAt: now, updatedBy: "admin-1" });

const descendantIds = (data: EventData, eventId: string) =>
  new Set(
    data.subevents
...
export const applyEventTransition = (
  ...
  if (!["DRAFT", "CLOSED", "ARCHIVED"].includes(next))
    return {
      ...data,
      events: data.events.map((item) =>
        item.id === eventId ? stamp({ ...item, status: next }, now) : item,
      ),
    };
  const subeventIds = descendantIds(data, eventId);
  const versionIds = new Set(
    data.subeventForms
      .filter((item) => subeventIds.has(item.subeventId))
      .map((item) => item.formVersionId),
  );
  ...
  return {
    ...
    subevents: data.subevents.map((item) =>
      subeventIds.has(item.id) ? stamp({ ...item, status: next }, now) : item,
    ),
    forms: data.forms.map((item) =>
      formIds.has(item.id) ? stamp({ ...item, status: next }, now) : item,
    ),
    ticketOptions: data.ticketOptions.map((item) =>
      subeventIds.has(item.subeventId) ? { ...item, status: next } : item,
    ),
  };
};
...
export const applySubeventTransition = (
  ...
  return {
    ...data,
    subevents: data.subevents.map((item) =>
      item.id === subeventId ? stamp({ ...item, status: next }, now) : item,
    ),
  };
};
...
export const applyFormTransition = (
  ...
  return {
    ...data,
    forms: data.forms.map((item) =>
      item.id === formId ? stamp({ ...item, status: next }, now) : item,
    ),
  };
};

export const isParticipantVisible = (status: LifecycleStatus) =>
  status === "UPCOMING" || status === "OPEN" || status === "CLOSED";
export const canRegister = (
  data: EventData,
  event: Pick<PrototypeEvent, "status">,
  subevent: Pick<PrototypeSubevent, "id" | "status">,
) =>
  event.status === "OPEN" &&
  subevent.status === "OPEN" &&
  subeventOpenBlockers(data, subevent).length === 0;
